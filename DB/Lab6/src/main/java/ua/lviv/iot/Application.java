...
public class Application {
    ...
    private static void readPlayers() throws SQLException{
        rs = statement.executeQuery("SELECT COUNT(*) FROM football_player");
        ...
        rs = statement.executeQuery("SELECT * FROM football_player");

        System.out.format("\n----------------------Table FootballPlayer --------------------\n");
        System.out.format("%-16s %-16s %-10s %-16s \n", "Last Name", "First Name", "Years XP", "Club");
        while (rs.next()) {
            ...
            // Simply Print the results
            System.out.format("%-16s %-16s %-10d %-16s \n", lastName, firstName, yearsOfExperience, clubName);
        }
    }
